...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setSummaryState: (summaryState: SummaryState) => set({ summaryState }),
  ...
  startGame: async () => {
    ...
    try {
      const backgroundResponse = await apiService.generateBackground({
        model_id: modelId,
      });
  ...
  finishGame: async () => {
    ...
    try {
      const resultResponse = await apiService.generateResult({
        session_id: currentSession.sessionId,
      });
      ...
      const lifeScore = Math.round(
        (resultResponse.radar_scores.financial +
          ...
          resultResponse.radar_scores.self_fulfillment) /
          5
      );

      // 生成成就與關鍵選擇
      const achievements = generateAchievements(resultResponse);
...
// 輔助函式：生成成就
function generateAchievements(result: ResultResponse): Achievement[] {
...
function generateKeyChoices(session: GameSession): string[] {
  return (session.eventHistory || []).map(
    (history, index) => `第${index + 1}個重要時刻：${history.outcome}`
  );
}
