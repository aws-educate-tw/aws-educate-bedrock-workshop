...
class MockGameState {
  private sessions: Map<string, {
    ...
    eventHistory: Array<{ event: StoryResponse; selectedOption: string; outcome: string }>;
  ...
  private mockEvents: StoryResponse[] = [
    ...
    {
      event_id: 'adult_2',
      event_description: '30歲的你面臨人生重要抉擇：是否要結婚定下來，還是繼續專注事業發展？',
      options: [
    ...
    {
      event_id: 'elder_1',
      event_description: '50歲的你開始思考人生下半場。你想要如何度過接下來的歲月？',
      options: [
  ...
  createSession(modelId: string): string {
    ...
    this.sessions.set(sessionId, {
      modelId,
      background: '你出生在一個普通的中產階級家庭，父母都是上班族。從小你就展現出好奇心旺盛的特質，喜歡探索新事物。',
  ...
  resolveEvent(sessionId: string, selectedOptionId: string): ResolveEventResponse {
    const session = this.getSession(sessionId);
    ...
    const outcome = outcomes[selectedOptionId as 'A' | 'B'][currentEvent.event_id as keyof typeof outcomes.A] || '你做出了選擇，人生繼續前進。';

    // 更新玩家狀態
    const stateChanges = this.calculateStateChanges(currentEvent.event_id, selectedOptionId);
    const newPlayerState = { ...session.playerState };
    
    Object.keys(stateChanges).forEach(key => {
      if (key !== 'traits') {
        (newPlayerState as any)[key] = Math.max(0, Math.min(100, (newPlayerState as any)[key] + stateChanges[key]));
      }
    });
  ...
  generateFinalResult(sessionId: string): ResultResponse {
    ...
    const radar_scores: RadarScores = {
      ...
      self_fulfillment: Math.floor((playerState.career + playerState.health + playerState.relationships) / 3)
    };

    // 生成最終總結
    const avgScore = (radar_scores.financial + radar_scores.career + radar_scores.health + radar_scores.relationships + radar_scores.self_fulfillment) / 5;
    
    let summary = `經過 ${session.eventHistory.length} 個重要抉擇，你的人生畫下了句號。`;
    
    if (avgScore >= 80) {
      summary += '你過著充實而成功的一生，在各個方面都取得了優異的成就。你的選擇展現了智慧與勇氣，為自己和他人創造了價值。';
    } else if (avgScore >= 60) {
      summary += '你的人生雖有起伏，但整體而言是平衡且有意義的。你在某些領域表現出色，同時也學會了接受生活的不完美。';
    } else {
      summary += '你的人生充滿挑戰，但每一次的困難都讓你變得更加堅強。雖然結果不盡如人意，但你的努力和堅持值得敬佩。';
    }

    const ending_type = avgScore >= 80 ? 'perfect' : avgScore >= 60 ? 'good' : 'challenging';
...
export const mockGameService = {
  generateBackground: (request: BackgroundRequest): BackgroundResponse => {
  ...
  generateStory: (request: StoryRequest): StoryResponse => {
  ...
  resolveEvent: (request: ResolveEventRequest): ResolveEventResponse => {
    return mockState.resolveEvent(request.session_id, request.selected_option_id);
  },

  generateResult: (request: ResultRequest): ResultResponse => {
